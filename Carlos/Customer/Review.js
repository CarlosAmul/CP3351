...
export default function ReviewsScreen({ review }) {
    ...
    useEffect(() => db.Users.listenOne(setUser, review.supportid), [])
    ...
    const linkToDetails = () => {
        console.log(req)
        navigation.navigate("Installations", { screen: 'DetailsScreen', params: { request: req } })
    } 
    return (
        <Card
            row
        ...
        >
            <View style={{ flexDirection: 'column' }}>
                <View style={{ margin: 5, padding: 5, textAlign: 'center' }}>
                    ...
                    <Text text80 grey20>{user ? 'Your review for ' + user.name : 'unknown'}</Text>
